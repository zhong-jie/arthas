...
public class SpringPropertyCommand extends AnnotatedCommand {
    ...
    public void process(CommandProcess process) {
        try {
            ...
            Map<String, SpringPropertiesVO> springDisplayName2SpringPropertiesVOMap = new HashMap<>(instances.length * 2);
            Map<String, String> child2ParentMap = new HashMap<>(instances.length * 2);
            for (Object applicationContext : instances) {
                ...
                Method getDisplayNameMethod = applicationContext.getClass().getMethod("getDisplayName");
                Object springDisplayName = getDisplayNameMethod.invoke(applicationContext);
                springDisplayName2SpringPropertiesVOMap.put(springDisplayName.toString(), springPropertiesVO);

                Method getParentMethod = applicationContext.getClass().getMethod("getParent");
                Object parent = getParentMethod.invoke(applicationContext);
                if (parent != null) {
                    Method parentGetDisplayNameMethod = parent.getClass().getMethod("getDisplayName");
                    Object parentSpringDisplayName = parentGetDisplayNameMethod.invoke(parent);
                    child2ParentMap.put(springDisplayName.toString(), parentSpringDisplayName.toString());
                } else {
                    child2ParentMap.put(springDisplayName.toString(), null);
                }
            }

            // 过滤掉parent的属性
            while (!child2ParentMap.isEmpty()) {
                Set<String> childSet = child2ParentMap.keySet();
                Collection<String> parentSet = child2ParentMap.values();
                Set<String> lastChildSet = childSet.stream().filter(child -> !parentSet.contains(child)).collect(Collectors.toSet());
                if (lastChildSet.isEmpty()) {
                    break;
                }

                for (String child : lastChildSet) {
                    String parent = child2ParentMap.get(child);
                    SpringPropertiesVO springPropertiesVO = springDisplayName2SpringPropertiesVOMap.get(child);
                    Map<String, String> childPropertyMap = springPropertiesVO.getProperties();
                    if (parent != null) {
                        Map<String, String> parentPropertyMap = springDisplayName2SpringPropertiesVOMap.get(parent).getProperties();

                        for (Map.Entry<String, String> entry : parentPropertyMap.entrySet()) {
                            String key = entry.getKey();
                            String parentValue = entry.getValue();
                            String childValue = childPropertyMap.get(key);
                            if (Objects.equals(parentValue, childValue)) {
                                childPropertyMap.remove(key);
                            }
                        }

                    }

                    if (childPropertyMap.isEmpty()) {
                        result.getSpringPropertiesVOList().remove(springPropertiesVO);
                    }
                    child2ParentMap.remove(child);
                }
            }
